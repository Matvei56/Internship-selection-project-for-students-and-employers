 Internship Selection Project  
Платформа зручного пошуку місця проходження практики для судентів.


## 🛠 Технології
- Python  
- Odoo 15  
- PostgreSQL  
- JavaScript  
- HTML / CSS  

 Основні задачі
 - Ведення реєстру підприємств (баз практики) та довготермінових угод.
- Прив’язка баз практики до спеціальностей/ОП, типів практик, періодів дії угод.
- Пошук/фільтрація та рекомендації “підходить моїй ОП”.
- Процес подачі/погодження заявки студента.
- Генерація DOCX/PDF: "Направлення на практику", "Угода про практику"
- Аналітика, звіти та експорт (для кафедри/адміністратора).
